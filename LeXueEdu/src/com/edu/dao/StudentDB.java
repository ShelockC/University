...
import com.edu.bean.StudentBean;
...
public class StudentDB {
	...
	public ArrayList getTeacherPage(int pageNow){//------学生分页给定页数的数据------------
		...
		conn=Conn.getConnect();
		String sql="select * from student where authentication='1' limit ?,?";
		try {
			ps=conn.prepareStatement(sql);
	...
	//查询单个学生的信息
	public StudentBean getStudentInfo(String tell){
		conn=Conn.getConnect();
		StudentBean bean=new StudentBean();
		...
		public boolean UpdateStudent(String name,String pid,String age,String sex,String email,String grade,String requestSex,String address,String status,String requestMoney,String tell){
		boolean tt=false;
		conn=Conn.getConnect();
		String sql = "update student set s_name='"+name+"',sex='"+sex+"',age='"+age+"',pid='"+pid+"',email='"+email+"',grade='"+grade+"',requestSex='"+requestSex+"',address='"+address+"',requestMoney='"+requestMoney+"',status='"+status+"'  where tell='"+tell+"'";  
	    ...
	    public boolean addOrder(String s_name,String t_name,String s_tell,String subject,String Rtime,String address,String method,String remark){
			boolean tt=false;
			conn=Conn.getConnect();
			String sql = "insert into reservation(s_name, t_name, s_tell, subject, Rtime, address, method, remark)values('"+s_name+"','"+t_name+"','"+s_tell+"','"+subject+"','"+Rtime+"','"+address+"','"+method+"','"+remark+"')";  
	        //String sql="update teacher set pid='"+pid+"'where id=1";
			//System.out.println("bb");
